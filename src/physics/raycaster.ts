...
export default class Raycaster {
   ...
   * @param length The length of the ray.
   */
  set(origin: vec2, direction: vec2, length: number) {
    this.origin = origin;
    this.direction = direction;
    this.length = length;

    this.end = vec2.fromValues(direction[0], direction[1]);
    vec2.scaleAndAdd(this.end, this.origin, this.end, this.length);
  }
  ...
  intersectRectCollider(box: RectCollider) {}
}
